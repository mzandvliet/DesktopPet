...
public class SystemInput
{
    ...
    public static bool GetKey(KeyCode key)
    {
        if (VK_KeyCodes.TryGetValue(key, out var value))
        {
            ...
            bool currState = IsBitSet(GetAsyncKeyState(value), 15); // == BUTTONDOWN
            _keyStates[key] = currState;
            return currState;
        }
        else
        {
            Debug.LogError($"GetKey: KeyCode {key} is not currently mapped");
        }
    ...
    public static bool GetKeyDown(KeyCode key)
    {
        if (VK_KeyCodes.TryGetValue(key, out var value))
        {
            ...
            bool prevState = _keyStates[key];
            bool currState = IsBitSet(GetAsyncKeyState(value), 15); // == BUTTONDOWN
            bool wasPressedThisFrame = (!prevState) && currState;
            _keyStates[key] = currState;
            return wasPressedThisFrame;
        }
        else
        {
            Debug.LogError($"GetKeyDown: KeyCode {key} is not currently mapped");
        }
    ...
    public static bool GetKeyUp(KeyCode key)
    {
        if (VK_KeyCodes.TryGetValue(key, out var value))
        {
            // return GetAsyncKeyState(value) == BUTTONUP;
            if (!_keyStates.ContainsKey(key))
            {
                _keyStates.Add(key, false);
            }
            ...
            bool prevState = _keyStates[key];
            bool currState = IsBitSet(GetAsyncKeyState(value), 15); // == BUTTONUP
            bool wasPressedThisFrame = prevState && (!currState);
            _keyStates[key] = currState;
            return wasPressedThisFrame;
        }
        ...
        {
            Debug.LogError($"GetKeyUp: KeyCode {key} is not currently mapped");
        }
    ...
    private static bool IsBitSet(short v, int pos)
    {
        return (v & (1 << pos)) != 0;
    }
